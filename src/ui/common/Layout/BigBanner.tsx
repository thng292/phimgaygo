...
const BigBanner: FC<BigBannerProps> = (props) => {
    if (props.ids.length === 0)
        return (
            <div
                id={"bigbanner"}
                className={
                    "aspect-video bg-gradient-to-t from-black via-gray-800 to-gray-800 animate-pulse"
                }
            ></div>
        );
    ...
    let currentTag =
        props.media_type[
            currentIndex <= props.media_type.length ? currentIndex : 0
        ];
    useEffect(() => {
            ...
            style={{
                backgroundImage: `linear-gradient(to top right, #222222dd, #42424200), url(${
                    window.innerWidth > 640
                        ? props.bannerFullURL[currentIndex]
                        : props.postersFullURL[currentIndex]
                })`,
        ...
        >
            <div className="absolute bottom-0 w-full">
                ...
                <TitlesRow
                    ...
                    genres={props.genres}
                    dates={props.date}
                    vote_avgs={props.vote_avgs.map((value) =>
                        value.toPrecision(2)
                    )}
                    imagesFullURL={
                    ...
                    onClickAction={(id, type) =>
                        setIndex(props.ids.findIndex((value) => value === id))
                    }
